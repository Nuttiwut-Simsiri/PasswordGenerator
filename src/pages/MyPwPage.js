...
const PasswordEl = ({ index, my_password, removeFn }) => {
    ...
    return (
        <tr className='bg-white border-b dark:bg-gray-800 dark:border-gray-700' ref={element}>
            ...
            <td className="py-4 px-6">
                <div>
                    <input readOnly={true} className='bg-transparent focus:outline-gray-900 focus:border-1 focus:bg-transparent focus:border-gray-500' type={show ? "text" : "password"} value={my_password.password}></input>
                </div>
            </td>
...
const MyPwPage = () => {
    ...
    const filterBy = (filteredData) => {
        ...
        const filteredPassword = filteredData.filter(
            (detail) => detail.username.includes(filterMsg) || detail.website.includes(filterMsg)
        );
    ...
    const removeItem = async (remove_id) => {
        ...
        let result = await window.Api.removePassword(remove_id);
        if (result.ok) {
    ...
    const getAllPassword = async () => {
        let allpw = await window.Api.getAllPassword();
        let AllPwList = Object.keys(allpw).map((key) => allpw[key]);
        console.log(AllPwList)
        setAllPassword(AllPwList)
        setMyPassword(AllPwList)
    }
            ...
            <div className='flex justify-center mt-8 overflow-x-auto relative shadow-md rounded-lg'>
                <table className='w-11/12 text-sm text-left text-gray-200 '>
                    ...
                    <tbody>
                        {
                            myPassword.map((el, i) => {
                                return <PasswordEl key={el.id} index={el.id} my_password={el} removeFn={handleRemove}></PasswordEl>
                            })
                        }
